# File automatically-generated by STM32CubeMX - Do not modify
set(ST_MULTICONTEXT MULTI_CONTEXT CACHE STRING "Type of multi-context")
#-----------------------Build CM0PLUS Project-----------------------#
if((${BUILD_CONTEXT} MATCHES .*CM0PLUS.*) OR (NOT DEFINED BUILD_CONTEXT))
    message("   Build context: " CM0PLUS)
    ExternalProject_Add(EnigmaEmbedded_CM0PLUS
        BINARY_DIR                  ${CMAKE_SOURCE_DIR}/CM0PLUS/build
        SOURCE_DIR                  ${PROJECT_SOURCE_DIR}/CM0PLUS
        PREFIX                      CM0PLUS
        CONFIGURE_HANDLED_BY_BUILD  true
        INSTALL_COMMAND             ""
        CMAKE_ARGS                  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON  -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE}  -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}
        BUILD_ALWAYS                true
    )

	set_property(DIRECTORY APPEND PROPERTY ADDITIONAL_CLEAN_FILES "${CMAKE_SOURCE_DIR}/CM0PLUS/build")
	set(ST_MULTI_CONTEXT_CM0PLUS_PROJECT_BUILD_TARGET ${CMAKE_SOURCE_DIR}/CM0PLUS/build/EnigmaEmbedded_CM0PLUS${CMAKE_EXECUTABLE_SUFFIX_CXX} CACHE FILEPATH "Path to cm0plus project target")
endif()
#-----------------------Build CM4 Project-----------------------#
if((${BUILD_CONTEXT} MATCHES .*CM4.*) OR (NOT DEFINED BUILD_CONTEXT))
    message("   Build context: " CM4)
    ExternalProject_Add(EnigmaEmbedded_CM4
        BINARY_DIR                  ${CMAKE_SOURCE_DIR}/CM4/build
        SOURCE_DIR                  ${PROJECT_SOURCE_DIR}/CM4
        PREFIX                      CM4
        CONFIGURE_HANDLED_BY_BUILD  true
        INSTALL_COMMAND             ""
        CMAKE_ARGS                  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON  -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE}  -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}
        BUILD_ALWAYS                true
    )

	set_property(DIRECTORY APPEND PROPERTY ADDITIONAL_CLEAN_FILES "${CMAKE_SOURCE_DIR}/CM4/build")
	set(ST_MULTI_CONTEXT_CM4_PROJECT_BUILD_TARGET ${CMAKE_SOURCE_DIR}/CM4/build/EnigmaEmbedded_CM4${CMAKE_EXECUTABLE_SUFFIX_CXX} CACHE FILEPATH "Path to cm4 project target")
endif()

